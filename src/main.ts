...
import {
  autodetectarLenguaje,
  expandirSiglasPsiquiatria,
  removerReferenciasYAutores,
...
function escanearFiltrosInteligentes() {
  ...
  for (const f of filesWithText) {
    const lines: string[] = String(f.localText || '').split(/\r?\n/).map((l: string) => l.trim()).filter((l: string) => l.length >= 8 && l.length <= 120);
    const uniqueInFile = new Set<string>(lines);
