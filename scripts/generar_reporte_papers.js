...
async function runReport() {
  ...
  md += `**Directorio base**: \`${rootDir}\`\n`;
  md += `**Total de documentos analizados**: ${allPdfPaths.length}\n\n`;
  ...
  for (const [folder, files] of folderGroups) {
    const folderName = folder === '.' ? 'Papers (Raíz)' : folder;
    md += `## 📁 Carpeta: \`${folderName}\` (${files.length} archivos)\n\n`;
    md += `| Estado | Archivo | Título Obtenido |\n`;
    md += `|---|---|---|\n`;
    ...
    for (const fullPath of files) {
      ...
      try {
        ...
        if (isDetected) {
          totalDetected++;
          folderDetected++;
          md += `| ✅ Detectado | \`${fileName}\` | **${detectedTitle.replace(/\|/g, '-')}** |\n`;
        } else {
          totalFallback++;
          md += `| 🔄 Fallback | \`${fileName}\` | *${fallbackTitle.replace(/\|/g, '-')}* |\n`;
        }
      } catch (err) {
        md += `| ❌ Error | \`${fileName}\` | *Error: ${err.message}* |\n`;
      }
    }
    md += `\n*Tasa de detección en esta subcarpeta: **${folderDetected}/${files.length} (${Math.round(folderDetected/files.length*100)}%)***\n\n---\n\n`;
  }
  
  md += `## 📊 Resumen Global\n\n`;
  md += `- **Total de Papers analizados**: ${totalProcessed}\n`;
  md += `- **Títulos detectados en portada / pág 1**: ${totalDetected} (${Math.round(totalDetected/totalProcessed*100)}%)\n`;
  md += `- **Títulos asignados por fallback limpio**: ${totalFallback} (${Math.round(totalFallback/totalProcessed*100)}%)\n`;
  
  fs.writeFileSync('d:\\drive\\psiquiatria\\trabajo\\CREACION DE APPS\\PDF TEXT TO VOICE\\pruebas\\reporte_titulos_papers.md', md, 'utf-8');
  console.log("Reporte generado exitosamente en pruebas/reporte_titulos_papers.md");
}
