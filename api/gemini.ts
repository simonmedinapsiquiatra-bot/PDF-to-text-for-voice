...
function promptDeSistema(action: string, lang: 'es' | 'en'): string {
  const en = lang === 'en';
...
function etiquetaDeEntrada(action: string): string {
...
function promptDeMetadatos(text: string): string {
...
const MODELO_GEMINI_POR_DEFECTO = 'gemini-3.5-flash';

const PROVEEDORES_OPENAI = {
  groq: {
...
const ORDEN_PROVEEDORES = ['gemini', ...Object.keys(PROVEEDORES_OPENAI)];

type Claves = Record<string, string | undefined>;
...
async function llamarProveedorOpenAI(
  endpoint: string,
  apiKey: string,
  models: string[],
  systemPrompt: string,
...
function jerarquiaGemini(modelPath: string): string[] {
  return [modelPath, ...CADENA_MODELOS_GEMINI].filter((m, i, arr) => arr.indexOf(m) === i);
}

async function ejecutarCascada(opciones: {
  orden: string[];
  claves: Claves;
...
// --- HANDLER PRINCIPAL (COMPATIBLE CON VERCEL SERVERLESS Y EXPRESS) ---
